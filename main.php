      ...
      <div class="row">
        <div class="col-xs-12">
          <div class="box box-primary">
            <div class="box-header label-primary">
              <h3 class="box-title">รายการอุปกรณ์กีฬา</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              <table id="stock_v" class="table table-bordered table-striped">
                <thead>
                <tr>
                  <th style="text-align:center;" width="5%">ลำดับ</th>
                  <!--<th style="text-align:center;" width="10%">ประเภท</th>-->
                  <th style="text-align:center;" width="15%">รูป</th>
                  <th style="text-align:center;" width="20%">ชื่ออุปกรณ์กีฬา</th>
                  ...
                  <th style="text-align:center;" width="20%">เบิก</th>
                </tr>
                </thead>
                <tbody>
                <?
					$num=1;
					$num_ream=0;
					$strSQL = "SELECT * FROM material ORDER BY name_material ASC  ";
					$stmt  = mysql_query($strSQL);
					while($row = mysql_fetch_array($stmt)){
						$strSQLu = "SELECT * FROM unit where id_unit = '".$row[id_unit]."' ";
						$stmtu  = mysql_query($strSQLu);
						$rowu = mysql_fetch_array($stmtu);
						
						$strSQL_ream = "SELECT sum(num_ream) as sum_ream FROM ream where id_mat = '".$row[id_mat]."' and status != 'N' and status != 'B' and status != 'BN' and status != 'BY' ";
                ...
                <tr>
                  ...
                  <td style="text-align:center;"><img src="<?=$row[pic];?>" style="width:50%;"></td>
                  <td><?=$row[name_material];?></td>
                  <td style="text-align:center;"><?=$rowu[name_unit];?></td>
                  <td style="text-align:center;"><?=$row[stock_qty];?></td>
                  <td style="text-align:center;"><?=$row[stock_qty]-$row_ream[sum_ream];?></td>
                  <td style="text-align:center;">
                  		<a href="form_ream.php?menu=ream&id_mat=<?=$row["id_mat"];?>" class="btn btn-warning"><i class="fa fa-hand-lizard-o"></i> เบิกอุปกรณ์กีฬา</a>
                  </td>
                </tr>
                <? $num++; } ?>
                </tbody>
        ...
        <div class="col-xs-12">
          <div class="box box-info">
            <div class="box-header label-info">
              <h3 class="box-title">รายการเบิกอุปกรณ์กีฬา</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
                ...
                <?
					...
					if($_SESSION["status"] == 'ADMIN'){
						$strSQLr = "SELECT * FROM ream 
									inner join material on (material.id_mat = ream.id_mat)  ";
					}else{
						$strSQLr = "SELECT * FROM ream 
									inner join material on (material.id_mat = ream.id_mat) where ream.id_user = '".$_SESSION["id_user"]."' ";
					}
